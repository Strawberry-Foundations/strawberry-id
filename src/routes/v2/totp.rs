...
pub async fn login_totp(
    ...
    let temp_token = jar
        .get_private("_strawberryid.temp_token")
        ...
        if let Some(user_data) = user_data {
            return if StrawberryIdTotp::check(&user_data.totp_secret.clone(), &form.totp_code) {
                let code = CORE
                    .write()
                    .await
                    .generate_code(user_data.clone(), CodeType::Website);
                ...
                if meta.redirect_after_login {
                    ...
                    AnyResponder::Redirect(Box::from(Redirect::to(format!("/v2/{}/account", lang))))
                }
...
pub async fn disable_totp(lang: &str, form: Form<DisableTotpForm>, state: &State<AppState>, jar: &CookieJar<'_>) -> AnyResponder {
    let strings = state.messages.get(lang).cloned().unwrap();
