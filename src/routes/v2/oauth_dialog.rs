...
pub async fn oauth_permit_dialog(lang: &str, service: &str, params: ApiParams, state: &State<AppState>, jar: &CookieJar<'_>) -> AnyResponder {
    ...
    match jar.get_private("_strawberryid.username") {
        ...
        None => return AnyResponder::Redirect(Box::from(Redirect::to(
            format!("/v2/{lang}/login?service={}&oauth=true&code={code}", meta.service)
        )))
    };
    ...
    let service_wants_to_login = strings.service_wants_to_login
...
pub async fn oauth_permit_dialog_post(lang: &str, service: &str, params: ApiParams, state: &State<AppState>, jar: &CookieJar<'_>) -> AnyResponder {
    let code = params.code;
